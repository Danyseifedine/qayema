                    ...
                    <form method="POST" action="{{ route('menu-owner.menus.store-or-update') }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" 
                                :value="old('name', $menu?->name)" placeholder="Enter your menu name" 
                        ...
                        <div>
                            ...
                            <x-text-input id="slug" name="slug" type="text" class="mt-1 block w-full" 
                                :value="old('slug', $menu?->slug)" placeholder="menu-url-slug" 
                                pattern="[a-z0-9]+(?:-[a-z0-9]+)*" />
                            <p class="mt-1 text-sm text-gray-500">Leave empty to auto-generate from menu name. Used in the menu URL.</p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select id="menu_style" name="menu_style" 
                                    ...
                                    <option value="restaurant" {{ old('menu_style', $menu?->menu_style) == 'restaurant' ? 'selected' : '' }}>Restaurant</option>
                                    <option value="home" {{ old('menu_style', $menu?->menu_style ?? 'home') == 'home' ? 'selected' : '' }}>Home Cook</option>
                                </select>
                                <x-input-error class="mt-2" :messages="$errors->get('menu_style')" />
                            </div>

                            <div>
                                <x-input-label for="dish_limit" :value="__('Dish Limit')" />
                                <x-text-input id="dish_limit" name="dish_limit" type="number" class="mt-1 block w-full" 
                                    :value="old('dish_limit', $menu?->dish_limit ?? 20)" 
                                    min="1" max="1000" required />
                                <p class="mt-1 text-sm text-gray-500">Maximum number of dishes allowed</p>
                                <x-input-error class="mt-2" :messages="$errors->get('dish_limit')" />
                            </div>
                        </div>
