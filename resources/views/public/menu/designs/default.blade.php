                                ...
                                <div
                                    ...
                                    @if (($settings['show_dish_image'] ?? true) && $dish->hasMedia('images'))
                                        <div class="relative h-48 overflow-hidden">
                                            ...
                                            @if (($settings['show_prices'] ?? true) && $dish->price && $pricePosition !== 'next_to_title')
                                                ...
                                                @endphp
                                                <div class="absolute {{ getPricePositionClasses($pricePosition) }} bg-white/95 backdrop-blur-sm px-3 py-1.5 rounded-lg shadow-lg border border-white/50">
                                    ...
                                    <div class="p-6">
                                        <div class="flex items-start justify-between mb-2 gap-4">
                                            <h3 class="text-lg font-bold text-gray-900 flex-1 min-w-0 pr-4">{{ $dish->name }}</h3>
                                            @if (($settings['show_prices'] ?? true) && $dish->price && ($settings['price_position'] ?? 'bottom_right') === 'next_to_title')
                                                @php
                                                    $price = $dish->price;
                                                    if (($settings['currency_enabled'] ?? false) && ($settings['exchange_rate'] ?? null)) {
                                                        $exchangeRate = (float) $settings['exchange_rate'];
