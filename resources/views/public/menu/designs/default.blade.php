        ...
        @if (($settings['enable_share'] ?? true) && ($settings['menu_design'] ?? 'default') === 'default')
            <div x-data="{ open: false }" class="fixed bottom-6 right-6 z-50 flex flex-col items-end">
                ...
                <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-200"
                    ...
                    class="mb-2 bg-white rounded-lg shadow-xl border border-gray-200 overflow-hidden min-w-[180px]">
                    <!-- Copy Link -->
                    <button @click="copyMenuLink(); open = false"
                        class="w-full px-4 py-2.5 text-left hover:bg-gray-50 transition-colors flex items-center gap-3 text-gray-700">
                    ...
                    @if ($user->phone)
                        <a href="#" @click.prevent="contactOnWhatsApp(); open = false"
                            class="w-full px-4 py-2.5 text-left hover:bg-gray-50 transition-colors flex items-center gap-3 text-gray-700 border-t border-gray-100">
                            ...
                            <span class="text-xs font-medium">Contact on WhatsApp</span>
                        </a>
                    @endif
                </div>
                <!-- Share Button -->
                <button @click="open = !open" type="button"
                    class="w-10 h-10 bg-indigo-600 hover:bg-indigo-700 text-white rounded-full shadow-lg flex items-center justify-center transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
