...
<head>
    ...
    <title>{{ $menu->name }} - {{ $user->restaurant_name ?? 'Menu' }}</title>
...
<body class="antialiased bg-gray-50">
    ...
    @if($user->hasMedia('cover_image'))
        <div class="relative h-64 md:h-96 overflow-hidden">
            <img src="{{ $user->getFirstMediaUrl('cover_image') }}"
                 alt="Cover Image"
                ...
                <div class="flex flex-col md:flex-row items-center md:items-start gap-6">
                    ...
                    <div class="flex-shrink-0">
                        @if($user->hasMedia('logo'))
                            <img src="{{ $user->getFirstMediaUrl('logo') }}"
                                 alt="Logo"
                                 class="w-24 h-24 md:w-32 md:h-32 rounded-full object-contain border-4 border-white shadow-lg">
                        @else
                            <div class="w-24 h-24 md:w-32 md:h-32 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center border-4 border-white shadow-lg">
                ...
                <div class="mb-12" id="category-{{ $category->id }}">
                    ...
                    <div class="mb-6">
                        <div class="flex items-center gap-4 mb-4">
                            ...
                            @else
                                <div class="w-16 h-16 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center shadow-md">
                                ...
                                <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300">
                                    ...
                                    <div class="p-6">
                                        <div class="flex items-start justify-between mb-2">
                                            ...
                                            @if($dish->price)
                                                <span class="text-2xl font-bold text-indigo-600">${{ number_format($dish->price, 2) }}</span>
                                            @endif
                                        </div>
    ...
    <script>
        ...
        window.addEventListener('beforeunload', function() {
            navigator.sendBeacon('/{{ $menu->slug }}/track-exit', JSON.stringify({
